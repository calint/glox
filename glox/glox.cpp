...
//			glutStrokeString(GLUT_STROKE_MONO_ROMAN,text);
	...
	void drawhud(){
		...
		char ac[200];
		sprintf(ac,"%02d:%02d:%02d:%03d",t.tm_hour,t.tm_min,t.tm_sec,tv.tv_usec/1000);//? ostream
		y=y-(dy>>2);pl(ac,y,100,1,.08f);
	}
	void draw(){
		static int frame=0;
...
//		glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
	...
	void keybd(const unsigned char key,const int x,const int y){
		char*ks=new char[2];
		char s[]={key,0};
